 ...
 * Accessible via admin-ajax.php?action=ius_qr&slug=...&nonce=...
 */
add_action('wp_ajax_ius_qr', function () {
    if (!current_user_can('manage_options')) {
        wp_die('', '', ['response' => 403]);
    }

    check_ajax_referer('ius_qr_nonce', 'nonce');
    ...
    $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM $ius_table WHERE slug = %s", $slug));
    if (!$row) {
        wp_die('', '', ['response' => 404]);
    }

    $base = trim((string) $row->base_path) !== '' ? $row->base_path : ius_get_base_path();
    $short_url = ius_build_short_url($base, $row->slug);

    $qr_api = 'https://api.qrserver.com/v1/create-qr-code/?size=140x140&data=' . rawurlencode($short_url);

    $response = wp_remote_get($qr_api, ['timeout' => 5, 'sslverify' => true]);
...
function ius_validate_destination_url($url) {
...
register_activation_hook(__FILE__, function () {
    ...
    ius_get_static_qr_plugin()->on_activate();
    flush_rewrite_rules();
});
...
function ius_get_static_qr_plugin() {
    global $ius_static_qr;
    return $ius_static_qr;
}
...
function ius_admin_page() {
    ...
    $current_tab     = isset($_GET['tab']) ? sanitize_key(wp_unslash($_GET['tab'])) : 'shortener';
    if (!in_array($current_tab, ['shortener', 'static_qr'], true)) {
    ...
    <h2 class="nav-tab-wrapper" style="margin-top:8px;">
        <a href="<?php echo esc_url(add_query_arg('tab', 'shortener', $base_page)); ?>" class="nav-tab <?php echo $current_tab === 'shortener' ? 'nav-tab-active' : ''; ?>">URL Shortener</a>
        <a href="<?php echo esc_url(add_query_arg('tab', 'static_qr', $base_page)); ?>" class="nav-tab <?php echo $current_tab === 'static_qr' ? 'nav-tab-active' : ''; ?>">Static QR Redirect</a>
    </h2>

<?php
    if ($current_tab === 'static_qr') {
        ius_get_static_qr_plugin()->render_tab();
        echo '</div> <!-- wrap end -->';
        return;
    }
    ...
    if (isset($_POST['ius_create']) && check_admin_referer('ius_create_nonce')) {
        ...
        if (!empty($_POST['expires_at'])) {
            $timestamp = strtotime(sanitize_text_field(wp_unslash($_POST['expires_at'])));
            if ($timestamp) {
        ...
        if (!$original) {
            echo '<div class="error"><p>Original URL must be a valid http or https URL.</p></div>';
        } else {
            // Auto-generate slug
            if ($slug === '') {
                do {
            ...
            if (ius_is_reserved_slug($slug)) {
                ...
                if ($exists > 0) {
                    ...
                    $inserted = $wpdb->insert($ius_table, [
                        'slug'         => $slug,
                    ...
                    if ($inserted === false) {
                        echo '<div class="error"><p>Short URL could not be created. Please try again.</p></div>';
                    } else {
...
?>
            ...
            <tr>
                ...
                <td>
                    <?php if ($short_url): ?>
                        <a href="<?php echo esc_url($short_url); ?>" target="_blank" rel="noopener noreferrer">
                ...
                <td>
                    ...
                    <form method="POST" style="margin-top:6px;">
                        ...
                        <input type="hidden" name="new_status" value="<?php echo esc_attr($r->is_active ? 0 : 1); ?>">
                        <button class="button button-small"><?php echo esc_html($r->is_active ? 'Disable' : 'Enable'); ?></button>
                    </form>
                </td>
                    ...
                    if ($short_url) {
                        $qr_src = add_query_arg(
                            [
                                'action' => 'ius_qr',
                                'slug'   => $r->slug,
                                'nonce'  => wp_create_nonce('ius_qr_nonce'),
                            ],
                            admin_url('admin-ajax.php')
                        );
                        echo '<img src="' . esc_url($qr_src) . '" alt="QR code for ' . esc_attr($short_url) . '" style="max-width:80px;height:auto;display:block;margin:0 auto;" />';
                    }
                    ?>
                </td>
...
add_action('template_redirect', function () {
    ...
    $slug = sanitize_title(get_query_var('ius_slug'));
    if (!$slug) {
        return;
    }
    ...
    if ($row) {
        ...
        // Validate the stored original URL before redirecting
        $original = ius_validate_destination_url($row->original_url);
        if (!$original) {
            global $wp_query;
            $wp_query->set_404();
            return;
        ...
        wp_redirect($original, 302, 'URL Shortener');
        exit;
    }
